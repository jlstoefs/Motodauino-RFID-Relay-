...
/////////////////////////////////////// FadeNoDelay (phading LED without Delay)/////////////////////////////////// 14
/*void FadeNoDelay (int GBR,int PulseLength) { //Led pulsing; PulseLength is the length of a halve cycle
 
  static int prevModulo1=0; 
  int Modulo1=0;
  static bool LedToggle=0;
  int LedValue;
  digitalWrite(LedPinsGBR[2], LED_OFF);  // Make sure Red LED is off
  Modulo1=millis()% PulseLength;
  LedValue=map(Modulo1,0,PulseLength-1,0,250);
  if (Modulo1<prevModulo1) LedToggle=1-LedToggle;      //cycle every A millis, while avoiding delay function
  if (LedToggle==0) analogWrite(LedPinsGBR[GBR],LedValue);  //increasing luminosity
  else analogWrite(LedPinsGBR[GBR],250-LedValue);     //decreasing luminosity
  prevModulo1=Modulo1;
}*/
///////////////////////////////////////// FadeDelay (phading LED using Delay)  /////////////////////////////////// 13
/*void FadeDelay (int GBR, int Delay) {

for (int i=0;i<3;i++)  //all Leds off
  {digitalWrite (LedPinsGBR [i],LED_OFF);}
for (int i=0; i<=50;i++)          // Led increasing
    {analogWrite(LedPinsGBR [GBR],i*5);
    delay(Delay);
    }
for (int j=50; j>=0;j--)          //Led fading
    {analogWrite(LedPinsGBR [GBR],j*5);
    delay(Delay);
    }
}*/
///////////////////////////////////////// cycleLeds (Leds) used in programMode ///////////////////////////////////
/*void cycleLeds(int A) { //A=length in millis of the LED_ON

  static int prevModulo=0; //pulse de la led bleue lentement
  static int Modulo=0;
  static byte BGR=0;
  prevModulo=Modulo;
  Modulo=millis()% A;
  if (Modulo<prevModulo) //chaque Ams acycle
      { digitalWrite(LedPinsGBR [(BGR+2)%3],LED_OFF); //turns off previous led in the loop
        digitalWrite(LedPinsGBR [BGR],LED_ON); //turns on the current Led
      //digitalWrite(LedPinsGBR [(BGR+1)%3],LED_OFF); should not be usefull as should already be turned off due to previous run
      BGR++;
      if (BGR==3) BGR=0; //BGR cycling btw 0 and 3
      }
}*/
///////////////////////////////////////// denied (LedPinsGBR[2]) ///////////////////////////////////
/*void denied() {

 for (int i=0;i<5;i++)                   //blinks red led 5 times
          {digitalWrite(LedPinsGBR[2], LED_ON);      
          delay(100);
          digitalWrite(LedPinsGBR[2], LED_OFF);   
          delay(100);}
}*/
